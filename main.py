    ...
    try:
        if len(sys.argv) > 0:
            ...
            for index, row in df.iterrows():
                ...
                command = f'ffmpeg -i "{movie_directory}\{row["movie_list"]}" -vn -q:a 0 -map a "{export_directory}\exported_audio\{extracted_mp3_audio}.mp3"'
                print(command)
                run_command(command)
                ...
                command_2 = f'''ffmpeg -i {export_directory}\exported_audio\{extracted_mp3_audio}.mp3 -af "{volume_enable}" "{export_directory}\exported_audio\{movie_silenced}"'''
                ...
                command_3 = f'''ffmpeg -i "{export_directory}\exported_audio\{movie_silenced}" {audio_input} -filter_complex "{adelay}amix=inputs={len(audio_files)}:duration=longest:dropout_transition=0" {movie_output_mp3}'''
